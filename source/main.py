...
def print_tiers(tiers: list, level: int):
    for roll in tiers:
        format_output.print_stat_tiers(*roll, level)

def check_valid_inputs(level: int, stats: list, num_rolls: int) -> bool:
    return 0 <= level <= 275 and 0 <= num_rolls <= 4

def input_list() -> list:
    temp_list = []
    for _ in range(4):
        temp_list.append(int(input('Enter the stats: (order: str, dex, int, luk)')))
    return temp_list
