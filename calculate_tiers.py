...
def calculate_tiers(level: int, value_list: list, num_rolls: int):
    ...
    if _finished_list(value_list):
        ...
        is_dual_value_at_lvl = partial(check_value.is_dual_value, level)
        bool_is_double_roll = list(map(is_dual_value_at_lvl, value_list))
        dual_values = queries.get_all_tiers_at_level(level, 2)
        if any(bool_is_double_roll):
            ...
            for i in range(len(value_list)):
                ...
                if type(temp_list[i]) == int:
                    ...
                    if temp_list[i] >= 0:
                        ...
                        result = [(dual_values.index(dual_value_subtracted)-1,dual_vs_index,i),
                                calculate_tiers(level, temp_list, num_rolls-1)]
                        if -2 not in result and _count_nested(result) == num_rolls:
                            return result
        else:
            solo_values = queries.get_all_tiers_at_level(level, 1)
            is_solo_value_at_lvl = partial(check_value.is_solo_value, level)
            ...
            if any(bool_is_solo_roll):
                ...
                result = [(solo_values.index(solo_value_removed)-1, solo_value_index, -1),
                        calculate_tiers(level, value_list, num_rolls-1)]
